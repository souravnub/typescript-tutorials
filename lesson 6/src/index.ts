...
const num = echo<number>(123); // or echo(123 as number)
...
const isObj = (i: any): boolean =>
    typeof i === "object" && !Array.isArray(i) && i !== null;

const isObjWithTypes = <T>(
    i: T
): { isObj: boolean; argType: string; val: T } => {
...
/*
using type interface with generic types
...
user['address']['suite']
*/
const getSecondNestedProperty = <T, K extends keyof T, N extends keyof T[K]>(
    users: T[],
    key: K,
    nestedKey: N
): T[K][N][] => {
